                        ...
                        <table class="table">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>{{$slider->id}}</td>
                                    <td style="width: 20%;">
                                        <img style="height: 200px;object-fit:contain" src="{{asset('storage/'.$slider->image)}}" class="img-fluid mb-2" />
                                    </td>
                                    <td>{!!$slider->title!!}</td>
                                    <td>{!!$slider->subtitle!!}</td>
                                    <td>
                                        <a href="{{route('admin.sliders.edit',$slider->id)}}" class="btn btn-primary">Edit</a>
                                        <form method="post" class="delete-from" action="{{route('admin.sliders.destroy',$slider->id)}}">
                                            @csrf
                                            @method('delete')
